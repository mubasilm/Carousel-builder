 ...
 * Extracts thesis and rewrites — does NOT paste blog paragraphs into slides.
 */
...
function makeHook(thesis, sourceType) {
  const contrast = CONTRASTS.find((c) => c.pattern.test(thesis));
  if (contrast) return { eyebrow: contrast.eyebrow, headline: trimWords(contrast.hook, 12) };

  if (sourceType === "competitive") {
    return { eyebrow: "The wedge", headline: trimWords(`The real comparison is not features. It is ${trimWords(thesis, 6)}.`, 12) };
  }
  if (sourceType === "framework") {
    return { eyebrow: "Revenue Activation", headline: trimWords(thesis, 12) };
  }
  return {
    eyebrow: "Revenue Activation",
    headline: trimWords(thesis.endsWith("?") ? thesis : `What if ${trimWords(thesis, 8).toLowerCase()}?`, 12),
  };
}
...
function synthesizeHeadline(sentence, index, sourceType) {
  ...
  const pool = templates[sourceType] || templates.thought_leadership;
  return trimWords(pool[index % pool.length], 10);
}
...
function bodyFromIdeas(ideas) {
  if (ideas.length <= 1) return trimWords(ideas[0] || "", 40);
  return ideas.map((line) => `• ${trimWords(line, 10)}`).join("\n");
}
...
export function synthesizeCarouselFromBlog({ blogText, title }) {
  ...
  const hook = makeHook(thesis, sourceType);

  const problemPara = paras[1] || paras[0] || "";
  const problemHeadline =
    sourceType === "competitive" ? "The old model breaks here" : "Where teams get stuck";
  const problemBody = trimWords(
    sentences(problemPara).find((s) => /struggle|gap|problem|fail|miss|without/i.test(s)) ||
  ...
  const insightSlides = insightBlocks.map((block, i) => {
    const headline = synthesizeHeadline(block, i, sourceType);
    const bullets = bulletsFromParagraph(block, 4);
    return {
      type: "insight",
      eyebrow: sourceType === "framework" ? `Lever ${i + 1}` : `Insight ${i + 1}`,
      headline,
      body: bodyFromIdeas(bullets.length > 1 ? bullets : [trimWords(block, 35)]),
      closing_line: "",
      footer: "",
  ...
  const slides = [
    ...
    ...insightSlides,
    {
      ...
      body: trimWords(takeawaySent, 40),
      closing_line: trimWords(thesis, 12),
      footer: "",
